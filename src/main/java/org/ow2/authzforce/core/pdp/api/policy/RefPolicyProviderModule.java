...
/**
 * This is the interface for all modules responsible for finding Policy(Set)s by their Policy(Set)IdReference in a specific way (e.g. from a specific policy repository).
 * <p>
 * Implements {@link Closeable} because it may may use resources external to the JVM such as a cache, a disk, a connection to a remote server, etc. for retrieving the policies. Therefore, these
 * resources must be release by calling {@link #close()} when it is no longer needed.
 * 
 */
public interface RefPolicyProviderModule extends RefPolicyProvider, Closeable
{
	...
	abstract class Factory<CONF_T extends AbstractPolicyProvider> extends JaxbBoundPdpExtension<CONF_T>
	{
		 ...
		 */
		public abstract RefPolicyProviderModule getInstance(CONF_T conf, XACMLParserFactory xacmlParserFactory, int maxPolicySetRefDepth, ExpressionFactory expressionFactory,
