...
public abstract class BaseStaticRootPolicyProviderModule implements StaticRootPolicyProviderModule
{
	 ...
	 * <p>
	 * Currently it has only one module ({@link RefPolicyProviderModule}) but may have multiple ones in the future. Therefore, this class should not be merged
	 * with {@link RefPolicyProviderModule} since it may be a one-to-many relationship later, and not a one-to-one as currently.
	 */
	private static final class BaseStaticRefPolicyProvider implements CloseableStaticRefPolicyProvider
	{
		 ...
		 *            refPolicyProvider module factory creating instance of {@link StaticRefPolicyProviderModule} from configuration defined by
		 *            {@code jaxbRefPolicyProvider}
		 * @param jaxbRefPolicyProvider
		 *            XML/JAXB configuration of RefPolicyProvider module
		 * @param maxPolicySetRefDepth
		 *            maximum depth of PolicySet reference chaining via PolicySetIdReference: PolicySet1 -> PolicySet2 -> ...; a strictly negative value means
		 *            no limit
		 ...
		 */
		private <CONF extends AbstractPolicyProvider> BaseStaticRefPolicyProvider(CONF jaxbRefPolicyProvider, RefPolicyProviderModule.Factory<CONF> refPolicyProviderModFactory, XACMLParserFactory xacmlParserFactory, ExpressionFactory expressionFactory, CombiningAlgRegistry combiningAlgRegistry, int maxPolicySetRefDepth, EnvironmentProperties environmentProperties) throws IllegalArgumentException
		{
			this(refPolicyProviderModFactory == null ? null : validate(refPolicyProviderModFactory.getInstance(jaxbRefPolicyProvider, xacmlParserFactory, maxPolicySetRefDepth, expressionFactory, combiningAlgRegistry, environmentProperties)));
		}
		...
		private static StaticRefPolicyProviderModule validate(RefPolicyProviderModule refPolicyProviderModule) throws IllegalArgumentException
		{
			...
			{
				throw new IllegalArgumentException("RefPolicyProviderModule arg '" + refPolicyProviderModule + "'  is not compatible with " + BaseStaticRefPolicyProvider.class + ". Expected: instance of " + StaticRefPolicyProviderModule.class + ". Make sure the PDP extension of type " + RefPolicyProviderModule.Factory.class + " corresponding to the refPolicyProvider in PDP configuration can create such instances.");
			}
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.ow2.authzforce.core.policy.RefPolicyProvider#findPolicy(java. lang. String, com.sun.xacml.VersionConstraints, java.lang.Class,
		 * java.util.Deque)
		 */
		@Override
		public TopLevelPolicyElementEvaluator get(TopLevelPolicyElementType refPolicyType, String idRef, VersionPatterns constraints, Deque<String> policySetRefChain, EvaluationContext evaluationCtx) throws IndeterminateEvaluationException
		{
			/*
		...
		@Override
		public StaticTopLevelPolicyElementEvaluator get(TopLevelPolicyElementType refPolicyType, String idRef, VersionPatterns constraints, Deque<String> policySetRefChain) throws IndeterminateEvaluationException
		{
			/*
			 * It is the responsability of the refPolicyProviderMod to update policySetRefChain and check against maxPolicySetRefDepth - using
			 * RefPolicyProvider.Utils class - whenever a PolicySetIdReference is found in resolved PolicySets
	 ...
	 *            {@code jaxbRefPolicyProviderConf}: PolicySet1 -> PolicySet2 -> ...; a strictly negative value means no limit. If and only if
	 *            {@code jaxbRefPolicyProviderConf == null}, this parameter is ignored.
	 * @param xacmlParserFactory
	 *            XACML Parser Factory
	 * @param refPolicyProviderModFactory
	 ...
	 *             does not implement {@link StaticRefPolicyProviderModule}
	 */
	protected <CONF extends AbstractPolicyProvider> BaseStaticRootPolicyProviderModule(ExpressionFactory expressionFactory, CombiningAlgRegistry combiningAlgRegistry, XACMLParserFactory xacmlParserFactory, CONF jaxbRefPolicyProviderConf, RefPolicyProviderModule.Factory<CONF> refPolicyProviderModFactory, int maxPolicySetRefDepth, EnvironmentProperties envProps) throws IllegalArgumentException
	{
		...
		} else
		{
			 ...
			 */
			this.refPolicyProvider = new BaseStaticRefPolicyProvider(jaxbRefPolicyProviderConf, refPolicyProviderModFactory, xacmlParserFactory, expressionFactory, combiningAlgRegistry, maxPolicySetRefDepth, envProps);
		}
	}
