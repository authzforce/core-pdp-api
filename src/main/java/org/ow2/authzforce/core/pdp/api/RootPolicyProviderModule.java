...
public interface RootPolicyProviderModule extends Closeable
{
	 ...
	 */
	IPolicyEvaluator getPolicy(EvaluationContext context) throws IndeterminateEvaluationException, IllegalArgumentException;
	 ...
	 * </pre>
	 */
	abstract class Static implements RootPolicyProviderModule
	{
		/**
		...
		@Override
		public final IPolicyEvaluator getPolicy(EvaluationContext context) throws IndeterminateEvaluationException, IllegalArgumentException
		{
	...
	abstract class Factory<ROOT_POLICY_PROVIDER_CONF extends AbstractPolicyProvider> extends JaxbBoundPdpExtension<ROOT_POLICY_PROVIDER_CONF>
	{
		 ...
		 *            support of PolicyReferences is disabled or this RootPolicyProvider module already supports these. Used as argument to
		 ...
		 *            (optional) refPolicyProvider module factory for creating a module instance from configuration defined by {@code jaxbRefPolicyProviderConf}
		 *            . May be null iff {@code jaxbRefPolicyProviderConf == null}. If not null, it is the responsibility of the root Policy Provider
		 *            implementation to use this and {@code jaxbRefPolicyProviderConf} as argument to instantiate the ref Policy Provider, and close it with
		 *            (@link RefPolicyProviderModule#close()) when it is done using it, in particular when closing the root policy provider created by this
		 *            factory (with {@link RootPolicyProviderModule#close()}).
		 * @param environmentProperties
		 *            PDP configuration environment properties
		 * 
		 * @return the root policy provider module instance
		 */
		public abstract <REF_POLICY_PROVIDER_CONF extends AbstractPolicyProvider> RootPolicyProviderModule getInstance(ROOT_POLICY_PROVIDER_CONF conf,
