...
public interface Expression<V extends Value>
{
	 ...
	 *            Individual Decision)request context
	 * @param mdpContext
	 * 	 the context of the Multiple Decision request that the {@code individualDecisionContext} belongs to if the Multiple Decision Profile is used.
	 * 
	...
	/**
	 * Get the constant value of this expression if it has a constant value, i.e. independent of the evaluation context (e.g. AttributeValue, VariableReference to AttributeValue...). This is
	 * equivalent to call {@link #evaluate(EvaluationContext, Optional)} with {@code individualDecisionContext == null} and {@code mdpContext == Optional.empty()}. This enables expression consumers to do optimizations, e.g. functions may pre-compile/pre-evaluate parts of
