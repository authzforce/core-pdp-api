...
/**
 * Attribute provider module that implements {@link Closeable} because it may may use resources external to the JVM such as a cache, a disk, a connection to a
 * remote server, etc. for retrieving the attribute values. Therefore, these resources must be released by calling {@link #close()} when it is no longer needed.
 */
public interface CloseableAttributeProviderModule extends AttributeProviderModule, Closeable
{
	...
	interface DependencyAwareFactory
	{
		 ...
		 */
		CloseableAttributeProviderModule getInstance(DatatypeFactoryRegistry attrDatatypeFactory, AttributeProvider depAttrProvider);
	}

	/**
	 * Builder that creates a dependency-aware AttributeProviderModule factory from parsing the attribute dependencies (attributes on which the module depends
	 * to find its own supported attributes) declared in the XML configuration (possibly dynamic).
	 * 
	 * @param <CONF_T>
	...
	abstract class FactoryBuilder<CONF_T extends AbstractAttributeProvider> extends JaxbBoundPdpExtension<CONF_T>
	{
		 ...
		 */
		public abstract DependencyAwareFactory getInstance(CONF_T conf);
	}
