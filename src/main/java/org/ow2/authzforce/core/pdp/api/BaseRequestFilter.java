...
public abstract class BaseRequestFilter implements RequestFilter
{
			...
			final boolean requireContentForXPath, final Processor xmlProcessor) throws UnsupportedOperationException
	{
		...
		if (allowAttributeDuplicates)
		{
			 ...
			 * possible Attribute with same meta-data except a defined Issuer), whereas the goal of 'allowAttributeDuplicates == false' is to use immutable Bags in the first place, i.e. avoid going
	...
	@Override
	public final List<? extends IndividualDecisionRequest> filter(final Request jaxbRequest, final Map<String, String> namespaceURIsByPrefix) throws IndeterminateEvaluationException
			{
		/*
	 ...
	 */
	public abstract List<? extends IndividualDecisionRequest> filter(List<Attributes> attributesList, JaxbXACMLAttributesParser xacmlAttrsParser, boolean isApplicablePolicyIdListReturned,
