...
public interface RefPolicyProvider
{
	...
	class Utils
	{
		...
		/**
		 * Appends a valid chain of policy references to another one. The check consists to look for circular references or excessive length resulting from
		 * appending the one to the other.
		 * 
		 ...
		 *             {@code policyRefChain2 == null}, or circular reference (same ID in both chains) detected or max length (sum of the lengths of the two
		 *             chains) is greater than {@code maxPolicyRefDepth}
		 */
		public static Deque<String> appendAndCheckPolicyRefChain(Deque<String> policyRefChain1, List<String> policyRefChain2, int maxPolicyRefDepth)
				throws IllegalArgumentException
		{
			...
			/*
			 * Check for circular reference (loop). We check only the policy ID because we consider that a mere reference back to the same ID is not allowed, no
			 * matter what the version is.
			 */
			for (final String nextPolicyId : policyRefChain2)
			{
				if (resultPolicyRefChain.contains(nextPolicyId))
				{
					throw new IllegalArgumentException("Invalid PolicySetIdReference: circular reference (loop) detected: " + resultPolicyRefChain + " -> "
							+ nextPolicyId);
				}
	 ...
	 * @throws IllegalArgumentException
	 *             The resolved policy is invalid. The policy Provider module may parse policies lazily or on the fly, i.e. only when the policy is
	 *             requested/looked for.
	 * @throws IndeterminateEvaluationException
	 *             if error determining a matching policy of type {@code policyType}
	 */
	<P extends IPolicyEvaluator> P get(Class<P> refPolicyType, String policyIdRef, VersionPatterns constraints, Deque<String> policySetRefChain)
