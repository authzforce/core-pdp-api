...
public final class DecisionResults
{
    private static final class ImmutableNotApplicableResult extends BaseDecisionResult
    {
        ...
        private ImmutableNotApplicableResult(final Optional<ImmutableXacmlStatus> status)
        {
            super(status);
        }
    ...
    private static abstract class ApplicableResult extends BaseDecisionResult
    {
        ...
        private ApplicableResult(final Optional<ImmutableXacmlStatus> status, final ImmutableList<PrimaryPolicyMetadata> applicablePolicyIdList)
        {
            super(status);
    ...
    private static final class ImmutableIndeterminateResult extends ApplicableResult
    {
        ...
        {
            super(Optional.of(cause.getTopLevelStatus()), applicablePolicyIdList);
            /*
    ...
    private static final class ImmutableDPResult extends ApplicableResult
    {
         ...
         */
        private ImmutableDPResult(final DecisionType decision, final Optional<ImmutableXacmlStatus> status, final ImmutableList<PepAction> pepActions, final ImmutableList<PrimaryPolicyMetadata> applicablePolicyIdList)
        {
     ...
     */
    public static final DecisionResult SIMPLE_PERMIT = new ImmutableDPResult(DecisionType.PERMIT, Optional.empty(), null, null);

    /**
     * Simple immutable Deny Decision result (no status, no obligation/advice)
     */
    public static final DecisionResult SIMPLE_DENY = new ImmutableDPResult(DecisionType.DENY, Optional.empty(), null, null);

    /**
     * Simple immutable NotApplicable Decision result (no status)
     */
    public static final DecisionResult SIMPLE_NOT_APPLICABLE = new ImmutableNotApplicableResult(Optional.empty());

    /**
     * Instantiates a Permit decision with optional PEP actions (obligations and advice).
     *
     * @param status                 status; even if decision is Permit/Deny, there may be a status "ok" (standard status in XACML 3.0) or internal error on attribute resolution but not resulting in Indeterminate
     *                               because of special combining algorithm ignoring such results (like deny-unless-permit) or MustBePresent="false"
     * @param pepActions             PEP actions (obligations/advices)
     * @param applicablePolicyIdList list of identifiers of applicable policies that contributed to this result. If not null, the created instance uses only an immutable copy of this list.
     * @return permit result, more particularly {@link #SIMPLE_PERMIT} iff {@code status  == null && pepActions == null}.
     */
    public static DecisionResult getPermit(final Optional<ImmutableXacmlStatus> status, final ImmutableList<PepAction> pepActions, final ImmutableList<PrimaryPolicyMetadata> applicablePolicyIdList)
    {
        assert status != null;
        if (status.isEmpty() && (pepActions == null || pepActions.isEmpty()) && (applicablePolicyIdList == null || applicablePolicyIdList.isEmpty()))
        {
     ...
     */
    public static DecisionResult getDeny(final Optional<ImmutableXacmlStatus> status, final ImmutableList<PepAction> pepActions, final ImmutableList<PrimaryPolicyMetadata> applicablePolicyIdList)
    {
        if (status.isEmpty() && (pepActions == null || pepActions.isEmpty()) && (applicablePolicyIdList == null || applicablePolicyIdList.isEmpty()))
        {
            return SIMPLE_DENY;
     ...
     *
     * @param status status; even if decision is NotApplicable, there may be a status "ok" (standard status in XACML 3.0) or internal error on attribute resolution but not resulting in Indeterminate
     *               because of special combining algorithm ignoring such results (like deny-unless-permit) or MustBePresent="false"
     * @return deny result, more particularly {@link #SIMPLE_DENY} iff {@code status  == null && pepActions == null}.
     */
    public static DecisionResult getNotApplicable(final Optional<ImmutableXacmlStatus> status)
    {
        if (status.isEmpty())
        {
     ...
     *
     * @param extendedIndeterminate  Extended Indeterminate value (XACML 3.0 Core, section 7.10). We use the following convention:
     *                               <ul>
     *                               <li>{@link DecisionType#DENY} means "Indeterminate{D}"</li>
     *                               <li>{@link DecisionType#PERMIT} means "Indeterminate{P}"</li>
     ...
     *                               </ul>
     * @param applicablePolicyIdList list of identifiers of applicable policies that contributed to this result. If not null, the created instance uses only an immutable copy of this list.
     * @param cause                  cause of the Indeterminate result
     * @return Indeterminate result
     * @throws IllegalArgumentException if {@code cause  == null}
     */
     ...
     *
     * @param extendedDecision       extended decision
     * @param pepActions             obligations/advice elements
     * @param applicablePolicyIdList list of identifiers of applicable policies that contributed to this result. If not null, the created instance uses only an immutable copy of this list.
            ...
            throws IllegalArgumentException
    {
        ...
        final Optional<ImmutableXacmlStatus> status = extendedDecision.getStatus();
        switch (decision)
        {
            case PERMIT:
